...
def index(request):
    if request.method == "POST":
        ...
        bks = booking(fname=fname, lname=lname, email=email,
...
def adminp(request):
    if 'form_rejected' in request.POST and request.method == "POST":
        ...
        p = booking.objects.filter(
            id=request.POST.get('object_id', '')).delete()
        print(p)
        ...
        approved_obj = approved.objects.create(fname=booking_obj.fname, lname=booking_obj.lname,
                                               email=booking_obj.email, city=booking_obj.city, state=booking_obj.state, pin=booking_obj.pin)
        booking_obj.delete()
